...
// Memory sections 4, 5, 6, scan_buf0, and scan_buf1 were defined and pre-allocated
// in the previous revision of this file. However, these memory sections are not
// used by Tank's wifi driver. Comment out these un-used memory sections and keep
// them in the file.  If want to use these memory sections, remove "//" from
// the flag below.
//#define CONFIG_BROADCOM_WIFI_RESERVED_MEM

#ifdef CONFIG_BCMDHD_USE_STATIC_BUF

#define WLAN_STATIC_DHD_PROT		0
...
#define WLAN_STATIC_DHD_BCM_CFG80211	100

#define PREALLOC_WLAN_SEC_NUM		4
#define PREALLOC_WLAN_BUF_NUM		160
#define PREALLOC_WLAN_SECTION_HEADER	24

#define WLAN_SECTION_SIZE_0	(PREALLOC_WLAN_BUF_NUM * 112)
...
#define WLAN_SECTION_SIZE_8	(PREALLOC_WLAN_BUF_NUM * 160)
#define WLAN_SECTION_SIZE_12	(PREALLOC_WLAN_BUF_NUM * 320)
#define WLAN_SECTION_SIZE_100 	(72 * 1024)
...
void *wlan_static_bcm_cfg80211_buf = NULL;
...
void *product_wifi_mem_prealloc(int section, unsigned long size)
{
	// KERN_INFO doesn't print any message.  Use KERN_ERR instead.
	printk(KERN_ERR "%s: section=%d size=%lu \n", __func__, section, size);

	if (section == WLAN_STATIC_DHD_PROT && wlan_mem_array[0].size > size) {
		return wlan_mem_array[0].mem_ptr;
	}

	if (section == WLAN_STATIC_DHD_INFO && wlan_mem_array[1].size > size) {
		return wlan_mem_array[1].mem_ptr;
	}

	if (section == WLAN_STATIC_DHD_WLFC_INFO && wlan_mem_array[2].size > size) {
		return wlan_mem_array[2].mem_ptr;
	}

	if (section == WLAN_STATIC_DHD_WLFC_HANGER && wlan_mem_array[3].size > size) {
		return wlan_mem_array[3].mem_ptr;
	}

	if (section == WLAN_STATIC_DHD_BCM_CFG80211 && WLAN_SECTION_SIZE_100 > size) {
		return wlan_static_bcm_cfg80211_buf;
	}

	printk(KERN_ERR "%s: return NULL for section=%d size=%lu \n", __func__, section, size);
	return NULL;

#ifdef CONFIG_BROADCOM_WIFI_RESERVED_MEM
	if (section == WLAN_STATIC_PKT_BUF) /* 4 */
		return wlan_static_skb;
	if (section == WLAN_STATIC_SCAN_BUF0) /* 5, DHD_PREALLOC_WIPHY_ESCAN0 */
		return wlan_static_scan_buf0;
	if (section == WLAN_STATIC_SCAN_BUF1) /* 6 */
		return wlan_static_scan_buf1;
	if ((section < 0) || (section > WLAN_STATIC_PKT_BUF))
		return NULL;

	if (wlan_mem_array[section].size < size) /* for array index 0 to 4 */
		return NULL;

	return wlan_mem_array[section].mem_ptr;
#endif
}
EXPORT_SYMBOL(product_wifi_mem_prealloc);
...
int __init tank_init_wifi_mem(void)
{
	...
	// Memory section 100 is for Broadcom's private DHD 80211 configuration.
	wlan_static_bcm_cfg80211_buf = kmalloc(WLAN_SECTION_SIZE_100, GFP_KERNEL);
	if (!wlan_static_bcm_cfg80211_buf) {
		goto err_mem_alloc;
	}
...
// fosmod_wifi_memory begin
#ifdef CONFIG_BCMDHD_USE_STATIC_BUF
	.mem_prealloc = product_wifi_mem_prealloc,
#endif
// fosmod_wifi_memory end
